...
window.onload = async (event) => {
  ...
  const locationList = [
    "All Locations",
    ...new Set(allPeeps.map((x) => x["Current City"])),
  ];
  const deptList = [
    "All Roles",
    ...new Set(allPeeps.map((x) => x["Department"])),
  ];

  const urlParams = new URLSearchParams(window.location.search);
  let myParam = urlParams.get("role");
  if (myParam === "All Roles") {
    myParam = "";
  }
  if (myParam) {
  ...
  locationDropdown.innerHTML =
    `<select id="locationDropdown" onchange='selectLocation()'>` +
    locationList
      .map((x) => {
  ...
  roleDropdown.innerHTML =
    `<select id="roleDropdown" onchange='selectRole()'>` +
    deptList
      .map((x) => {
        return `<option value='${x}' ${
          x === filter.Department ? "selected" : ""
        }>${x}</option>`; // '${x === filter.Department}'
      })
      .join("") +
    "</select>";
};
...
const selectLocation = (e) => {
  ...
  val === "All Locations"
    ? delete filter["Current City"]
    : (filter["Current City"] = val);
...
const toggleRelocate = () => {
  filter["Open to Relocate"] === true
    ? delete filter["Open to Relocate"]
    : (filter["Open to Relocate"] = true);
  const people = filtering(everyone);
  displayPeeps(people);
};
...
const displayPeeps = (peeps) => {
  ...
  const inactiveContainer = document.getElementById("inactive_container");

  const inactiveEl = document.getElementById("inactive");
  const activePeep = peeps.filter((x) => !x["Found a Job"]);
  const inactivePeep = peeps.filter((x) => x["Found a Job"]);
  if (activePeep.length > 0) {
    appEl.innerHTML = activePeep.map((p) => styling(p)).join("");
  } else {
    appEl.innerHTML = `<h3>No match found.</h3>`;
  }

  if (inactivePeep.length > 0) {
    inactiveContainer.removeAttribute("style", "display: none");
    inactiveEl.innerHTML = inactivePeep.map((p) => styling(p)).join("");
  } else {
...
const styling = (p) => {
  ...
  const photo =
    (p["Your Profile Photo"] &&
      p["Your Profile Photo"][0]["thumbnails"].large) ||
    [];
  ...
  const displaySuperpower = superpower
    ? `<strong>Superpower</strong>: ${superpower}`
    : "";
  const displayLinkedIn = linkedIn
    ? `<li><a href=${linkedIn} target="_blank"><img src="images/linkedin-in.svg" alt="Linkedin" />Linkedin</a></li>`
    : "";
  const displayWebsite = website
    ? `<li><a href=${website} target="_blank"><img src="images/globe.svg" alt="Website" />${trimUrl(
        website
      )}</a></li>`
    : "";
  const displayGithub = github
    ? `<li><a href=${github} target="_blank"><img src="images/github.svg" alt="Github" />Github</a></li>`
    : "";
  const displayTwitter = twitter
    ? `<li><a href=${twitter} target="_blank"><img src="images/twitter.svg" alt="Twitter" />Twitter</a></li>`
    : "";
  const displayDribbble = dribbble
    ? `<li><a href=${dribbble} target="_blank"><img src="images/dribble.svg" alt="Dribble" />Dribbble</a></li>`
    : "";
  const displayCity = city ? `${city}` : "";
  const displayRemote = remote
    ? `<span class="relocate">Open to Remote</span>`
    : "";
  const displayRelocate = relocate
    ? `<span class="relocate">Can relocate</span>`
    : "";
